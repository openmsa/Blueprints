...
# Install Blueprints {{{
COPY --chown=1000:1000 . /opt/fmc_repository/Blueprints
RUN install_repo_deps.sh /opt/fmc_repository/Blueprints/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/Blueprints/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
